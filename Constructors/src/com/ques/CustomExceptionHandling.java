...
public class CustomExceptionHandling {
   ...
   // Custom exceptions (optional but aligns with the class name)
   ...
   public static void main(String[] args) {
       ...
       // Regex: one word OR two words with single space; total length >= 5

       final String userPattern = "^(?=.{5,}$)[A-Za-z]+(?:\\s[A-Za-z]+)?$";

       final Pattern userPtrn = Pattern.compile(userPattern);

       // Password: 8–20, lower+upper+digit+special, no whitespace
       ...
       try (Scanner sc = new Scanner(System.in)) {
           ...
           String userName = sc.nextLine().trim().replaceAll("\\s+", " "); // normalize spaces
           ...
           try {
               ...
               return; // stop early if username invalid
